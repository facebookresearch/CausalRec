...
def train_and_evaluate_ple2d(model, data, num_epochs=10, batch_size=256):
    """Training loop for PLE2D"""
    np.random.seed(RANDOM_SEED)
    torch.manual_seed(RANDOM_SEED)
...
def evaluate_ple2d_all_scenarios():
    """Evaluate PLE2D on all scenarios"""
    ...
    print("\nEvaluating PLE2D across all scenarios...")
    print("-" * 60)
    ...
    for scenario_key, data in scenarios.items():
        try:
            ...
            detailed_results = train_and_evaluate_ple2d(
                model,
                data,
...
if __name__ == "__main__":
    results = evaluate_ple2d_all_scenarios()
