...
def generate_data(
    ...
    noise_scale=0.1,    # Try increasing this for no-confounding scenarios
    seed=42,
    verbose=False
):
    """
    ...
    noise_correlation : float
        Correlation of noise terms, only used if num_outcomes=2
    noise_scale : float
    ...
    for k in range(num_treatments):
        if confounding_level > 0:
            ...
            alpha_k = beta_0 + (confounding_level * base_effect)
        else:
            # No confounding: tiny random noise around balanced base rates
            alpha_k = beta_0
        alpha_list.append(alpha_k)
    alpha_np = np.vstack(alpha_list).T
    
    # Convert to torch tensor and apply softmax directly
    # Remove temperature scaling to avoid complications
    alpha_torch = torch.FloatTensor(alpha_np)
    ...
    gamma_m = np.random.normal(1, 0.1, size=(num_outcomes, d))
    
    # Scale treatment effects based on number of treatments
    scale_factor = 1.0 if num_treatments == 2 else 0.5
    delta_m = np.random.normal(0, 0.1 * scale_factor, size=(num_outcomes, num_treatments))
    alpha_0 = np.random.normal(0, 0.1 * scale_factor, size=num_outcomes)
    alpha_t = np.random.normal(4, 0.1 * scale_factor, size=(num_outcomes, num_treatments))
    ...
    # Add noise
    if num_outcomes == 2:
        noise_cov = noise_scale**2 * np.array([
            [1.0, noise_correlation],
            [noise_correlation, 1.0]
        ])
        noise = np.random.multivariate_normal(np.zeros(2), noise_cov, size=N)
    else:
        noise = noise_scale * np.random.randn(N, 1)
    Y += noise
...
def analyze_component_correlations(M=2, N=3000):
    ...
    for p in p_values:
        data = generate_data(N=N, outcome_correlation=p, num_outcomes=M, verbose=False)
...
def generate_all_scenarios(N=10000, d=10, num_outcomes=2, num_treatments=3,
                         confounding_level=0.7, outcome_correlation=0.5, 
                         base_seed=42):
