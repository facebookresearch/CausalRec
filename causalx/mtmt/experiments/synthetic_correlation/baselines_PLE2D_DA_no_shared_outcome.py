...
from DGP_correlation import generate_all_scenarios
from model.model import PLE2D_DA
...
def train_and_evaluate_ple2d_da(model, data, num_epochs=60, batch_size=256, da_weight=1.0, loss_th=1000.0,lr=0.002):
    """Training loop for PLE2D with Distribution Alignment using Wasserstein/MMD distance"""
    np.random.seed(RANDOM_SEED)
    torch.manual_seed(RANDOM_SEED)
...
def evaluate_ple2d_da_all_scenarios():
    """Evaluate PLE2D_DA on all synthetic scenarios"""
    scenarios = generate_all_scenarios()
    results = {}
    
    scenario_desc = {
         'A': 'Multiple T, Multiple O, With Conf 3.0, Correlation 0.0',
         'B': 'Multiple T, Multiple O, With Conf 3.0, Correlation 0.1',
         'C': 'Multiple T, Multiple O, With Conf 3.0, Correlation 0.5',
         'D': 'Multiple T, Multiple O, With Conf 3.0, Correlation 1.0'
    }
    
    
    # Define scenario-specific hyperparameters
    #scenario_params = {
    #    'A': {'da_weight': 10, 'num_epochs': 250},    # Multiple outcomes + treatments
    #    'B': {'da_weight': 10, 'num_epochs': 250},     # Most complex case
    #    'C': {'da_weight': 10, 'num_epochs': 200},     # Most complex case
    #    'D': {'da_weight': 10, 'num_epochs': 200}     # Most complex case
    #}
    scenario_params = {
        'A': {'da_weight': 10, 'num_epochs': 200, 'lr': 0.002},    # Multiple outcomes + treatments #200
        'B': {'da_weight': 10, 'num_epochs': 200, 'lr': 0.002},     # Most complex case #200
        'C': {'da_weight': 10, 'num_epochs': 200, 'lr': 0.002},     # Most complex case #200
        'D': {'da_weight': 10, 'num_epochs': 220, 'lr': 0.002}     # Most complex case #10
    }
    print("\nEvaluating PLE2D_DA across all scenarios...")
    print("-" * 60)
    ...
    for scenario_key, data in scenarios.items():
        try:
            ...
            detailed_results = train_and_evaluate_ple2d_da(
                model, 
                data,
...
if __name__ == "__main__":
    results = evaluate_ple2d_da_all_scenarios()
