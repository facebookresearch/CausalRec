...
class BRCausalModel(TimeVaryingCausalModel):
    ...
    def __init__(self, args: DictConfig,
        ...
        self.initial_balancing_method = args.exp.initial_balancing_method
        self.final_balancing_method = args.exp.final_balancing_method
        self.balancing_switch_epoch = args.exp.balancing_switch_epoch
        self.current_balancing_method = self.initial_balancing_method

        #self.balancing = args.exp.balancing
        self.clustering_method = args.exp.clustering_method
        self.alpha = args.exp.alpha  # Used for gradient-reversal
        self.update_alpha = args.exp.update_alpha
        self.num_clusters = args.exp.number_clusters
        self.alpha_wass = args.exp.alpha_wass
        self.alpha_wass_epoch = args.exp.alpha_wass_epoch
        self.alpha_wass_growth = args.exp.alpha_wass_growth  # Incremental growth factor for each epoch
        self.alpha_wass_max = args.exp.alpha_wass_max  # Maximum limit for alpha_wass
        self.loss_sinkhorn = SamplesLoss("sinkhorn", p=1, blur=0.05,diameter=100)
        self.num_timepoints = args.exp.num_timepoints
        self.gap_epoch = args.exp.gap_epoch
        # Initialize variables for clustering
        self.gmm_model = None
        self.compute_wasserstein_loss_this_epoch = False



    def configure_optimizers(self):

        if self.current_balancing_method == 'subgroup' and not self.hparams.exp.weights_ema:
            optimizer = self._get_optimizer(list(self.named_parameters()))
        ...
        elif self.current_balancing_method == 'subgroup' and self.hparams.exp.weights_ema:
            non_treatment_head_params = [k for k in dict(self.named_parameters())]
            assert len(non_treatment_head_params) == len(list(self.named_parameters()))
            non_treatment_head_params = [(k, v) for k, v in dict(self.named_parameters()).items()
                                         if k in non_treatment_head_params]
            non_treatment_head_params = list(self.named_parameters())
        ...
        elif self.current_balancing_method == 'grad_reverse' and not self.hparams.exp.weights_ema:  # one optimizer
            optimizer = self._get_optimizer(list(self.named_parameters()))
    ...
    def on_epoch_start(self):
        ...
        Computes GMM clustering if Wasserstein loss is to be calculated in this epoch.
        """
        # Switch balancing method if necessary
        if (
            self.current_epoch >= self.balancing_switch_epoch
            and self.current_balancing_method != self.final_balancing_method
        ):
            self.current_balancing_method = self.final_balancing_method
            rank_zero_info(f"Switching balancing method to {self.final_balancing_method} at epoch {self.current_epoch}")

        # For the subgroup method, compute cluster labels for Wasserstein loss
        #if self.current_balancing_method == 'subgroup':
            #start_epoch = self.num_epoch_start_wass  # Start computing Wasserstein loss from a specific epoch
        #    gap_epoch = self.gap_epoch
        #    if self.current_epoch % gap_epoch == 0:
        #        self.compute_wasserstein_loss_this_epoch = True
        #        self.compute_cluster_labels()
        #    else:
        #        self.compute_wasserstein_loss_this_epoch = False

            # For the subgroup method, compute cluster labels for Wasserstein loss
        if self.current_balancing_method == 'subgroup':
            # Check if 'Decoder' is in the class name
            print(self.__class__.__name__)
            if 'CRNDecoder' in self.__class__.__name__:
                # Do not compute cluster labels for the decoder
                self.compute_wasserstein_loss_this_epoch = False
            else:
                # Proceed as usual for the encoder
                gap_epoch = self.gap_epoch
                if self.current_epoch % gap_epoch == 0:
                    self.compute_wasserstein_loss_this_epoch = True
                    self.compute_cluster_labels()
    ...
    def compute_cluster_labels(self):
        """
        Computes the GMM or KMeans clustering on embeddings from the training data.
        """
        self.eval()  # Set model to evaluation mode
        with torch.no_grad():
            embeddings_per_timepoint = {t: [] for t in range(self.num_timepoints)}
            data_loader = self.train_dataloader()
            print("timepoint_clustering")
            for i, batch in enumerate(data_loader):
                batch = {k: v.to(self.device) for k, v in batch.items()}
                _, _, embeddings = self(batch)  # embeddings shape: [batch_size, num_timepoints, embedding_dim]
                for t in range(self.num_timepoints):
                    embeddings_t = embeddings[:, t, :]  # Shape: [batch_size, embedding_dim]
                    embeddings_per_timepoint[t].append(embeddings_t)

            # For each timepoint, fit GMM or KMeans
            self.clustering_models = {}
            for t in range(self.num_timepoints):
                emb_list = embeddings_per_timepoint[t]
                if emb_list:
                    emb_avg = torch.cat(emb_list, dim=0)  # Shape: [total_samples, embedding_dim]
                    # Fit clustering model
                    if self.clustering_method == 'GMM':
                        model = GaussianMixture(n_components=self.num_clusters, random_state=0)
                        model.fit(emb_avg.detach().cpu().numpy())
                    elif self.clustering_method == 'KMeans':
                        model = KMeans(n_clusters=self.num_clusters, random_state=0)
                        model.fit(emb_avg.detach().cpu().numpy())
                    self.clustering_models[t] = model
                else:
                    # Handle case where there is no data for this timepoint
                    self.clustering_models[t] = None
    ...
    def training_step(self, batch, batch_ind, optimizer_idx=0):
        ...
        if optimizer_idx == 0:  # grad reversal or domain confusion representation update
            if self.current_balancing_method == 'subgroup':
                treatment_pred, outcome_pred, representations = self(batch)
            elif self.hparams.exp.weights_ema:
            ...
            if self.current_balancing_method == 'subgroup':
                print("subgroup_avg")
                wasserstein_loss = torch.tensor(0.0, device=embeddings.device)

                print("compute_wasserstein",self.compute_wasserstein_loss_this_epoch)

                if self.compute_wasserstein_loss_this_epoch:
                    treatments = batch['current_treatments']
                    num_timepoints = treatments.size(1)
                    num_treatments = treatments.size(2)  # Assuming last dimension is number of treatments

                    for t in range(num_timepoints):
                        model = self.clustering_models.get(t, None)
                        if model is None:
                            continue  # Skip if no clustering model for this timepoint
                        
                        emb_avg_list = []
                        for j in range(num_treatments):
                            mask_j = treatments[:, t, j] == 1
                            if torch.any(mask_j):
                                emb_j = embeddings[mask_j, t, :]
                                emb_avg_list.append(emb_j)

                        if emb_avg_list:
                            emb_avg = torch.cat(emb_avg_list, dim=0)
                            emb_avg_cpu = emb_avg.detach().cpu().numpy()
                            
                            # Get cluster labels using either GMM or KMeans
                            cluster_labels_avg = model.predict(emb_avg_cpu)
                            cluster_labels_avg = torch.from_numpy(cluster_labels_avg).long().to(embeddings.device)
                            
                            for j in range(num_treatments):
                                mask_j = treatments[:, t, j] == 1
                                if torch.any(mask_j):
                                    emb_j = embeddings[mask_j, t, :]
                                    emb_j_cpu = emb_j.detach().cpu().numpy()
                                    
                                    # Get cluster labels using either GMM or KMeans
                                    cluster_labels_j = model.predict(emb_j_cpu)
                                    cluster_labels_j = torch.from_numpy(cluster_labels_j).long().to(embeddings.device)

                                    for cluster in range(self.num_clusters):
                                        cluster_mask_avg = cluster_labels_avg == cluster
                                        cluster_mask_j = cluster_labels_j == cluster
                                        emb_cluster_avg = emb_avg[cluster_mask_avg]
                                        emb_cluster_j = emb_j[cluster_mask_j]
                                        if emb_cluster_avg.nelement() > 0 and emb_cluster_j.nelement() > 0:
                                            proportion_j = emb_cluster_j.size(0) / (emb_cluster_j.size(0) + emb_cluster_avg.size(0))
                                            # Compute Wasserstein distance for matched clusters
                                            wasserstein_loss += proportion_j*self.loss_sinkhorn(emb_cluster_avg, emb_cluster_j)

                    # Normalize the Wasserstein loss by the number of comparisons
                    num_comparisons = num_treatments * num_timepoints * self.num_clusters
                    wasserstein_loss /= num_comparisons if num_comparisons > 0 else 1
                else:
                    wasserstein_loss = torch.tensor(0.0, device=embeddings.device)
                ...
                if self.alpha_wass_epoch:
                    # Increase alpha_wass slightly with each epoch
                    epoch_factor = self.alpha_wass + self.alpha_wass_growth * current_epoch 
                    alpha_wass_epoch = min(epoch_factor, self.alpha_wass_max)
            
                if not self.alpha_wass_epoch:
                    # Keep alpha_wass constant
                    loss = mse_loss + self.alpha_wass * wasserstein_loss  # + bce_loss
                else:
                    # Increase alpha_wass slightly with each epoch
                    loss = mse_loss + alpha_wass_epoch * wasserstein_loss  # + bce_loss

                print("wasserstein_loss",wasserstein_loss)
                print("loss",loss)
                self.log(f'{self.model_type}_train_loss', loss, on_epoch=True, on_step=False, sync_dist=True)
                self.log(f'{self.model_type}_train_wass_loss', wasserstein_loss, on_epoch=True, on_step=False, sync_dist=True)
            ...
            elif self.current_balancing_method == 'grad_reverse':
                bce_loss = self.bce_loss(treatment_pred, batch['current_treatments'].double(), kind='predict')
            elif self.current_balancing_method == 'domain_confusion':
                ...
                #print("optimizer_idx 0 br_treatment_outcome_head alpha",self.br_treatment_outcome_head.alpha)
            else:
                raise NotImplementedError()
            ...
            bce_loss = self.bce_loss(treatment_pred, batch['current_treatments'].double(), kind='predict')
            if self.current_balancing_method == 'domain_confusion':
                #print("optimizer_idx 1 br_treatment_outcome_head alpha",self.br_treatment_outcome_head.alpha)
                bce_loss = self.br_treatment_outcome_head.alpha * bce_loss
    ...
    def test_step(self, batch, batch_ind, **kwargs):
        if self.current_balancing_method == 'subgroup' and self.hparams.exp.weights_ema:
            with self.ema_non_treatment.average_parameters():
        ...
        if self.current_balancing_method == 'grad_reverse':
            bce_loss = self.bce_loss(treatment_pred, batch['current_treatments'].double(), kind='predict')
        elif self.current_balancing_method == 'domain_confusion':
            bce_loss = self.bce_loss(treatment_pred, batch['current_treatments'].double(), kind='confuse')
